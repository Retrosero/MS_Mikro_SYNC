...
Uygulamanın GitHub'a `lisans_server` adıyla (veya istediğiniz başka bir isimle) yüklendiğinden emin olun. Proje içerisinde oluşturduğumuz `Dockerfile` ve `.dockerignore` dosyaları uygulamanın Coolify üzerinde Docker tabanlı olarak en verimli şekilde derlenmesini sağlayacaktır.
